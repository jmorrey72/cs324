...
int main(int argc, char *argv[]) {
	...
	while(1) {
		...
		if (op_code == 0) {
			if (af == AF_INET) {
				sendto(socketfd, nonce_buf, 4, 0, (struct sockaddr *) &ipv4addr_remote, server_addr_len);
			} else {
				sendto(socketfd, nonce_buf, 4, 0, (struct sockaddr *) &ipv6addr_remote, server_addr_len);
			}
			recvfrom(socketfd, response_buffer, RESPONSE_SIZE, 0, NULL, NULL);
		} 
			...
			if (af == AF_INET) {
				ipv4addr_remote.sin_port = htons(op_param_host_order);
			} else {
				ipv6addr_remote.sin6_port = htons(op_param_host_order);
			}
			s = getaddrinfo(server, portAsStr, &hints, &result);
			if (s != 0) {
				perror("getaddrinfo");
			...
			if (af == AF_INET) {
				...
				ipv6addr_local.sin6_port = htons(op_param_host_order); // specific port
				bzero(ipv6addr_local.sin6_addr.s6_addr, 16); // any/all local addresses
				if (bind(socketfd, (struct sockaddr *)&ipv6addr_local, sizeof(struct sockaddr_in6)) < 0) {
					perror("bind()");
				}
			}
			...
			if (af == AF_INET) {
				if (sendto(socketfd, nonce_buf, 4, 0, (struct sockaddr *) &ipv4addr_remote, server_addr_len) < 0) {
					perror("sendto");
				}
			} else {
				if (sendto(socketfd, nonce_buf, 4, 0, (struct sockaddr *) &ipv6addr_remote, server_addr_len) < 0) {
					perror("sendto");
				}
			}
			recvfrom(socketfd, response_buffer, RESPONSE_SIZE, 0, NULL, NULL);
