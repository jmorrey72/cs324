/*
Computation times:
    1 core: parallel time - 29.373160
            total time - 33.82
    2 cores: parallel time - 15.258165
             total time - 19.67
    4 cores: parallel time - 8.927609
             total time - 13.38
    8 cores: parallel time - 4.863555
             total time - 9.09
    16 cores: parallel time - 3.337604
              total time - 7.71
    32 cores: parallel time - 2.616549
              total time - 7.07

Questions:

1. 20 cores
2. it decreases by a little less than 50%
3. S4 = 3.29
4. 32 threads
5. Ran out of cores
6. 33.82/13.38 = 2.53
7. 2.53/4 = .63
8. A significant portion of the program is not parallelized
9. (13.38/33.82 - 1)/(1/3.29 - 1) = .87
10. .13(33.82) = 4.40 seconds
*/
...
int main(int argc, char* argv[])
{
  ...
  start = omp_get_wtime(); 
  #pragma omp parallel for private(i,y,x) lastprivate(k) 
  for (j = 0; j < yres; j++) {
    y = ymax - j * dy;
    for(i = 0; i < xres; i++) {
      double u = 0.0;
