...
int main(int argc, char *argv[]) {
	...
	while(1) {
		int nread;
		fflush(stdin);
		nread = fread(&chunk[size], CHUNK, 1, stdin);
		if (nread == 0) 
			break;
		if (nread > 0) {
			size = size + nread;
	...
	printf("%d\n", size);
	write(STDOUT_FILENO, &inbuf[0], size);
	printf("\n");
	while(1) {
		int nrecv;
		int location = 0;
		if ((size - location) >= CHUNK) {
			nrecv = write(sfd, &inbuf[location], CHUNK);
		} else {
			nrecv = write(sfd, &inbuf[location], (size - location));
		}
		if (nrecv < 0) {
			perror("write");
			exit(0);
