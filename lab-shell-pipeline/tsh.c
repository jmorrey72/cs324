...
int mydup2(int oldfd, int newfd) {
    if (dup2(oldfd, newfd) == -1) {
        fprintf(stderr, "Dup2 error; oldfd: %d, newfd: %d\n", oldfd, newfd);
        exit(1);
    }
    return newfd;
}
...
void eval(char *cmdline) 
{
    char *envp[] = { NULL };
    char *argv[MAXLINE];
    int cmds[MAXARGS] = {[0 ... (MAXARGS - 1)] = -1};
        ...
        if (pid == 0) {
            ...
            if (numCmds > 1) {
                if (i == 0) {
                    ...
                    myclose(oldPipefd[1]);
                    mydup2(oldPipefd[0], 0);
                    mydup2(pipefd[1], 1);
                    myclose(pipefd[1]);
                    myclose(oldPipefd[0]);
                }
            }
            myexecve(argv[cmds[i]], &argv[cmds[i]], envp);
            return;
        } else {
            pids[i] = pid;
            if (i == 0)
